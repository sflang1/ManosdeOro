...
<br>
					<hr>
<br>
					<h1 id="titulos_noticias">Noticias actualmente en el sistema:</h1>
					<?php
						$noticias=$ndao->loadAll($conn);
						if(count($noticias)<=4)
						{
							echo("No hay noticias actualmente en el sistema");
						}
						else
						{
							echo("<table>");
							echo("<th id='strings_editar_rojo'>Título de la noticia</th>");
							echo (	"<br>");
							echo("<th id='strings_editar_rojo'>Descripción de la noticia</th>");
							for($i=4;$i<count($noticias);$i++)
							{
								echo("");
								echo("<tr>");
								echo("<td id='strings_editar'>".$noticias[$i]->getTitulo()."</td>");
								echo("<td id='strings_editar'>".$noticias[$i]->getContenido()."</td>");
								echo("<td><form method='POST' action='modificarNoticia.php'><td><input type='hidden' name='id' value='".$noticias[$i]->getIdnoticia()."'><input type='submit' value='Modificar noticia'></form></td><");
								echo("<td><form method='POST' action='eliminarNoticia.php'><td><input type='hidden' name='id' value='".$noticias[$i]->getIdnoticia()."'><input type='submit' value='Eliminar noticia'></form></td>");
								echo("</tr>");
							}
							echo("</table>");
						}
