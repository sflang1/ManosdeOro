...
<html>
	<head>
		<title>Manos de Oro - Admo Stands</title>
			...
			<section id="sec6">
				<center>
					...
					<?php
						...
						{
							?>
								<h1 id="strings_editar">No hay solicitudes para stands actualmente.</h1>
							<?php
						}
						else
						{
							echo("<table>");
							echo("<th id='strings_editar'>Nombre del artesano</th>");
							echo("<th id='strings_editar'>Evento </th>");
							echo("<th id='strings_editar'>Stand solicitado</th>");
							echo("<th id='strings_editar'></th>");
							echo("<th id='strings_editar'></th>");
							for($i=0;$i<count($solicitudes);$i++)
							{
								echo("<tr id='strings_editar'>");
								$artesano=$adao->getObject($conn,$solicitudes[$i]->getIdartesano());
								if((1<=$solicitudes[$i]->getIdstand())&&($solicitudes[$i]->getIdstand()<=60))
								{
									$evento=1;
									$stand=$solicitudes[$i]->getIdstand();
								}
								if((61<=$solicitudes[$i]->getIdstand())&&($solicitudes[$i]->getIdstand()<=80))
								{
									$evento=2;
									$stand=$solicitudes[$i]->getIdstand()-60;
								}
								if((81<=$solicitudes[$i]->getIdstand())&&($solicitudes[$i]->getIdstand()<=100))
								{
									$evento=3;
									$stand=$solicitudes[$i]->getIdstand()-80;
								}
								echo("<td id='strings_editar'>".utf8_encode($artesano->getNombre())."</td>");
								echo("<td id='strings_editar'>".$evento."</td>");
					...
					<script type="text/javascript" src="confirmarStands.js"></script>
				</center>
			</section>
			<footer>		
