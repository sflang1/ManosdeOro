<?php
 ...
 /**
  * Noticias Data Access Object (DAO).
  * This class contains all database handling that is needed to 
  * permanently store and retrieve Noticias object instances. 
  */
...
class NoticiasDao {
    ...
    function load($conn, $valueObject) {
          ...
          $sql = "SELECT * FROM noticias WHERE (idNoticia = ".$valueObject->getIdNoticia().") "; 
    ...
    function create($conn, $valueObject) {
          ...
          $sql = $sql."'".$valueObject->getContenido()."') ";
    ...
    function save($conn, $valueObject) {
          ...
          $sql = $sql."contenido = '".$valueObject->getContenido()."'";
          $sql = $sql." WHERE (idNoticia = ".$valueObject->getIdNoticia().") ";
    ...
    function delete($conn, $valueObject) {
          ...
          $sql = "DELETE FROM noticias WHERE (idNoticia = ".$valueObject->getIdNoticia().") ";
          $result = $this->databaseUpdate($conn, $sql);
    ...
    function searchMatching($conn, $valueObject) {
          ...
          if ($valueObject->getIdNoticia() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND idNoticia = ".$valueObject->getIdNoticia()." ";
          }
          ...
          if ($valueObject->getContenido() != "") {
              if ($first) { $first = false; }
              $sql = $sql."AND contenido LIKE '".$valueObject->getContenido()."%' ";
          }


          $sql = $sql."ORDER BY idNoticia ASC ";
    ...
    function singleQuery($conn, $sql, $valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setIdNoticia($row[0]); 
                   $valueObject->setTitulo($row[1]); 
                   $valueObject->setContenido($row[2]); 
          } else {
    ...
    function listQuery($conn, $sql) {
          ...
          while ($row = $conn->nextRow($result)) {
               ...
               $temp->setIdNoticia($row[0]); 
               $temp->setTitulo($row[1]); 
               $temp->setContenido($row[2]); 
